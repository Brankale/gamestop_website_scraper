...
fun parse(gameId: Int): GameNew? {
    val url = "$SCHEME$DOMAIN_IT$SUB_DIRECTORY$gameId"
    val document = Jsoup.connect(url).get()
    return getGame(document, gameId)
}

private fun getGame(document: Document, gameId: Int): GameNew? {
...
private fun buildGame(gameId: Int, dtos: List<GamePageDto>): GameNew? {
    if (dtos.isNotEmpty()) {
        val first = dtos[0]
